...
{
	CHECK_PARAMS(16, "CreateDynamic3DTextLabel");
	...
	Utility::addToContainer(textLabel->worlds, static_cast<int>(params[11]));
	Utility::addToContainer(textLabel->interiors, static_cast<int>(params[12]));
	Utility::addToContainer(textLabel->players, static_cast<int>(params[13]));
	textLabel->comparableStreamDistance = amx_ctof(params[14]) < STREAMER_STATIC_DISTANCE_CUTOFF ? amx_ctof(params[14]) : amx_ctof(params[14]) * amx_ctof(params[14]);
	textLabel->streamDistance = amx_ctof(params[14]);
	Utility::addToContainer(textLabel->areas, static_cast<int>(params[15]));
	textLabel->priority = static_cast<int>(params[16]);
