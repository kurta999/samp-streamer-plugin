...
{
	CHECK_PARAMS(15, "CreateDynamicVehicle");
	...
	vehicle->comparableStreamDistance = amx_ctof(params[13]) < STREAMER_STATIC_DISTANCE_CUTOFF ? amx_ctof(params[13]) : amx_ctof(params[13]) * amx_ctof(params[13]);
	vehicle->streamDistance = amx_ctof(params[13]);
	Utility::addToContainer(vehicle->areas, static_cast<int>(params[14]));
	vehicle->priority = static_cast<int>(params[15]);
	vehicle->worldID = static_cast<int>(params[10]) < 0 ? 0 : static_cast<int>(params[10]);
	vehicle->position = vehicle->spawn.position;
	vehicle->angle = vehicle->spawn.angle;
	vehicle->color = vehicle->spawn.color;
	vehicle->paintjob = 3; // In GTA, you can use 0 - 3 IDs for paintjobs, 4 is invalid
...
cell AMX_NATIVE_CALL Natives::IsDynamicVehicleStreamedIn(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		{
			return sampgdk::IsVehicleStreamedIn(i->second, static_cast<int>(params[2]));
		}
	}
	return 0;
}
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleSpawnInfo(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		Utility::storeIntegerInNative(amx, params[6], v->second->spawn.color[0]);
		Utility::storeIntegerInNative(amx, params[7], v->second->spawn.color[1]);
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehicleSpawnInfo(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		v->second->spawn.color[0] = static_cast<int>(params[6]);
		v->second->spawn.color[1] = static_cast<int>(params[7]);
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehiclePos(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::SetVehiclePos(i->second, amx_ctof(params[2]), amx_ctof(params[3]), amx_ctof(params[4]));
		}
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleZAngle(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::GetVehicleZAngle(i->second, &angle);
		}
		else
		{
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleRotationQuat(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::GetVehicleRotationQuat(i->second, &quat[0], &quat[1], &quat[2], &quat[3]);
		}
		else
		{
			quat = v->second->quat;
		}
		Utility::storeFloatInNative(amx, params[2], quat[0]);
		Utility::storeFloatInNative(amx, params[3], quat[1]);
		Utility::storeFloatInNative(amx, params[4], quat[2]);
		Utility::storeFloatInNative(amx, params[5], quat[3]);
		return 1;
	}
	return 0;
}
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehicleZAngle(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::SetVehicleZAngle(i->second, angle);
		}
		else
		{
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehParamsForPlayer(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::SetVehicleParamsForPlayer(i->second, static_cast<int>(params[2]), static_cast<int>(params[3]) != 0, static_cast<int>(params[4]) != 0);
		}
		return 1;
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehicleToRespawn(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		v->second->paintjob = 3; // In GTA, you can use 0 - 3 IDs for paintjobs, 4 is invalid
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::SetVehicleToRespawn(i->second);
			sampgdk::SetVehiclePos(i->second, v->second->position[0], v->second->position[1], v->second->position[2]);
...
cell AMX_NATIVE_CALL Natives::AddDynamicVehicleComponent(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::AddVehicleComponent(i->second, componentid);
		}
		return 1;
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::RemoveDynamicVehicleComponent(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::RemoveVehicleComponent(i->second, componentid);
		}
		return 1;
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::ChangeDynamicVehicleColor(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		v->second->color[0] = static_cast<int>(params[2]);
		v->second->color[1] = static_cast<int>(params[3]);
		boost::unordered_map<int, int>::iterator i = core->getData()->internalVehicles.find(v->first);
		if (i != core->getData()->internalVehicles.end())
		{
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleColor(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		Utility::storeIntegerInNative(amx, params[2], v->second->color[0]);
		Utility::storeIntegerInNative(amx, params[3], v->second->color[1]);
		return 1;
	}
	return 0;
}
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleHealth(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::GetVehicleHealth(i->second, &health);
		}
		else
		{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (v2 != core->getData()->vehicles.end())
		{
			...
			if (i != core->getData()->internalVehicles.end())
			{
				...
				{
					sampgdk::AttachTrailerToVehicle(i2->second, i->second);
					return 1;
				}
			}
		}
...
cell AMX_NATIVE_CALL Natives::DetachTrailerFromDynamicVeh(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			return sampgdk::DetachTrailerFromVehicle(i->second);
		}
	}
	return 0;
}
...
cell AMX_NATIVE_CALL Natives::IsTrailerAttachedToDynamicVeh(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			return sampgdk::IsTrailerAttachedToVehicle(i->second);
		}
	}
	return 0;
}
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleTrailer(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			int trailerid = sampgdk::GetVehicleTrailer(i->second);
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehComponentInSlot(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			ret = sampgdk::GetVehicleComponentInSlot(i->second, slot);
		}
		else
		{
			try
			{
				ret = v->second->carmods[slot];
			}
			catch (...)
			{
				ret = 0;
...
cell AMX_NATIVE_CALL Natives::RepairDynamicVehicle(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::RepairVehicle(i->second);
		}
		else
		{
			v->second->health = 1000.0f;
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleVelocity(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::GetVehicleVelocity(i->second, &velocity[0], &velocity[1], &velocity[2]);
		}
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehicleVelocity(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::SetVehicleVelocity(i->second, velocity[0], velocity[1], velocity[2]);
			return 1;
		}
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehAngularVelocity(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::SetVehicleAngularVelocity(i->second, velocity[0], velocity[1], velocity[2]);
			return 1;
		}
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::GetDynamicVehicleDamageStatus(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::GetVehicleDamageStatus(i->second, &panels, &doors, &lights, &tires);
		}
		else
		{
...
cell AMX_NATIVE_CALL Natives::SetDynamicVehicleVirtualWorld(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::SetVehicleVirtualWorld(i->second, static_cast<int>(params[2]));
		}
		return 1;
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::GetPlayerSurfingDynamicVehID(AMX *amx, cell *params)
{
	...
	int internalid = sampgdk::GetPlayerSurfingVehicleID(static_cast<int>(params[1]));
	if (internalid)
	{
	...
	if (p != core->getData()->players.end())
	{
		...
		if (v != core->getData()->vehicles.end())
		{
			...
			if (i == core->getData()->internalVehicles.end())
			{
				...
				{
					sampgdk::UpdateVehicleDamageStatus(internalID, v->second->panels, v->second->doors, v->second->lights, v->second->tires);
				}
				if (v->second->health != 1000.0f)
				{
					sampgdk::SetVehicleHealth(internalID, v->second->health);
				}
				...
				{
					sampgdk::SetVehicleParamsEx(internalID, v->second->params.engine, v->second->params.lights, v->second->params.alarm, v->second->params.doors, v->second->params.bonnet, v->second->params.boot, v->second->params.objective);
				}
				...
				{
					sampgdk::SetVehicleParamsCarDoors(internalID, v->second->params.cardoors.driver, v->second->params.cardoors.passenger, v->second->params.cardoors.backleft, v->second->params.cardoors.backright);
				}
				...
				{
					sampgdk::SetVehicleParamsCarWindows(internalID, v->second->params.carwindows.driver, v->second->params.carwindows.passenger, v->second->params.carwindows.backleft, v->second->params.carwindows.backright);
				}
				sampgdk::PutPlayerInVehicle(playerid, internalID, seatid);
				core->getData()->internalVehicles.insert(std::make_pair(v->first, internalID));
			}
			else
			{
				sampgdk::PutPlayerInVehicle(playerid, i->second, seatid);
			}
...
cell AMX_NATIVE_CALL Natives::GetPlayerDynamicVehicleID(AMX *amx, cell *params)
{
	...
	int internalid = sampgdk::GetPlayerVehicleID(static_cast<int>(params[1]));
	if (internalid)
	{
		for (boost::unordered_map<int, int>::iterator i = core->getData()->internalVehicles.begin(); i != core->getData()->internalVehicles.end(); ++i)
		{
...
cell AMX_NATIVE_CALL Natives::IsDynamicVehicleOccupied(AMX *amx, cell *params)
{
	...
	if (i != core->getData()->internalVehicles.end())
	{
		...
		{
			if (sampgdk::GetPlayerVehicleID(p->first) == i->second) return 1;
		}
	}
	return 0;
}
...
cell AMX_NATIVE_CALL Natives::PlayerSpectateDynamicVehicle(AMX *amx, cell *params)
{
	...
	if (v != core->getData()->vehicles.end())
	{
		...
		if (i != core->getData()->internalVehicles.end())
		{
			sampgdk::PlayerSpectateVehicle(static_cast<int>(params[1]), i->second, static_cast<int>(params[3]));
		}
		return 1;
	}
	return 0;
